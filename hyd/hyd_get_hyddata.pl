...
#
# required files:
#	-/usr/local/cchdo_perl_lib/exchange_get_sumdata.pl
#
# params: (0,1,2)
#	0- $hyd file name
#	1- $sum file name
...
use strict;
require "/usr/local/cchdo_perl_lib/exchange_get_sumdata.pl";
...
sub hyd_get_hyddata {
    ...
    ##
    ( $ret_expocode, $num_header_lines ) =
      get_header_info( \@buffer, \@col_name );
    ...
    for ( my $col = 0 ; $col <= $#col_name ; $col++ ) {
        ...
        if ( $units[$col] =~ /UMOL\/L/ ) {
            my $er_col = $col + 1;
            print STDOUT
              "WARNING: UMOL/L unit found on col $er_col of .HYD file\n";
        }
    ...
    for ( $rec_no = 0 ; $rec_no < $buffer_length ; $rec_no++ ) {
	    ...
	    # The flag must be > 1 to count
            if ( $asterisk[$i] =~ /\*\*\*/ ) {
                if (
                    !( defined( $qualt_flags_1[$qualt_count] ) )
                    || (   ( $num_qualt == 2 )
                        && ( !( defined( $qualt_flags_2[$qualt_count] ) ) ) )
                  )
                {
                    print STDOUT
"The bottle file does not have the correct number of flags.\n";
                    die "Could not continue conversion.\n";
                }
                ...
                if ( $num_qualt == 2 ) {
                    ...
                    {
                        $tmp_hash{$stn_no}{$cast_no}{$press}{$rec_no}
                          {$qualt_name} = $qualt_flags_1[$qualt_count];
                    }
                    else {
                        $tmp_hash{$stn_no}{$cast_no}{$press}{$rec_no}
                          {$qualt_name} = $qualt_flags_2[$qualt_count];
                    }
                }
                else {
    ...
    foreach $stn_no (@stn_no_order) {
        if ( exists( $tmp_hash{$stn_no} ) ) {
            foreach $cast_no ( sort { $a <=> $b }
                keys( %{ $tmp_hash{$stn_no} } ) )
            {
                foreach $press ( sort { $a <=> $b }
                    keys( %{ $tmp_hash{$stn_no}{$cast_no} } ) )
                {

                    #print STDERR "STN: $stn_no CAST: $cast_no PRES: $press\n";
                    foreach $rec_no ( sort { $a <=> $b }
                        keys( %{ $tmp_hash{$stn_no}{$cast_no}{$press} } ) )
                    {
                        foreach $elem (
                            keys
                            %{ $tmp_hash{$stn_no}{$cast_no}{$press}{$rec_no} } )
