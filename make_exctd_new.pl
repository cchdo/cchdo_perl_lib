...
# 					corrected paths
#
# S. Diggs		2007.08.08	MAJOR REVISION
#					Added code changes in all routines
#					related to this program to include
#					transmissometer and fluorometer
#					values.
#
#------------------------------------------------------------------------
my $software_version = "CTD_Exchange_Encode_v2.0(CCHDO/Diggs)";
...
require $basedir . "write_exctd_data_new.pl";
...
foreach my $ctd_loop_filename (@ctdfile_names)	{
	...
	for (my $s=0 ; 	$s <= $#{$sum_hash{'CASTNO'}} ; $s++)	{
		...
		if 
	 	  (
			($ctd_header{'stnnbr'} == $sum_hash{'STNNBR'}[$s])
			and
			($ctd_header{'castno'} == $sum_hash{'CASTNO'}[$s])
		   )							
		 # or							
	 	 #(
	...
	&write_exctd_data_new(\%ctd_header, \%ctd_data, 'W', $software_version);
}
#
#------------------------> END LOOP
#
