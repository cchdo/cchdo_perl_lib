...
#
# PURPOSE:
# This script takes a HYD file and converts it to the improved exchange file
# format.  The conversion is based on version 2000.07.18 of the Improved Exchange
# Format manual written by James Swift and edited by Stephen Diggs.
#
# The script takes in two arguments, the HYD and SUM files, extracts the needed
# data into a hash using an external procedure.  The data is then checked for
# known problems and the desired parameters are printed to the new exchange file.
# The desired parameters are stored in an array in hyd_excahnge_params.pl.  The
# array is in the order and contains the fields that were chosen by
...
#			 from getParams()
#	@exact_headers	-used to store the parameters in order to print in
...
#				local vars to top
# modified 9.29.2000
# modified 2006.05.03 (S. Diggs: Linux port to new CCHDO machine)
# M. Shen (2008-08-01)	Cleaned out unused libraries and made parameter format
#			and order depend on exchangeParams.txt
#--------------------------------------------------------------------------------

use strict;
use File::stat;         #for getting the file mod. dates
use Time::localtime;    #to convert the mod. dates to readable format

require "/usr/local/cchdo_perl_lib/get_whpo_timestamp.pl";
require "/usr/local/cchdo_perl_lib/round_float.pl";
require "/usr/local/cchdo_perl_lib/hyd/hyd_get_hyddata.pl";
...
if ( exists( $hyd_hash{'NITRAT'} ) && !( exists( $hyd_hash{'NITRIT'} ) ) ) {
    print STDOUT
      "WARNING: NITRAT column but no NITRIT column found in the HYD file\n";
}
...
open( OUTPUT, "> $output_name" )
  or die "Could not open $output_name for writing. \n";

#
# get the WHPO stamp for line 1
#
...
my ( $exact_headers_ref, $format_ref ) = getParams();
@exact_headers = @$exact_headers_ref;
%format        = %$format_ref;
...
# stored in a hash that is returned by the getParams()
# function.
#
...
for ( my $i = 0 ; $i < $hyd_size ; $i++ ) {
    ...
    foreach my $header (@hyd_header) {
        if ( $header =~ /_FLAG_W/ ) {
            push( @data_output,
                sprintf( "%1d", $hyd_hash{$header}{'data'}[$i] ) );
        }
        #
        ...
        #
            ...
            #
            # If the precision != 0 - round the headerent and
            # store rounded value back into the hash.
            #
            my $localPrecision;
            if ( ( $localPrecision = getPrecision( %format, $header ) ) != 0 ) {
                $hyd_hash{$header}{'data'}[$i] = round_float( 
			$hyd_hash{$header}{'data'}[$i], $localPrecision );
            }

            push( @data_output,
                sprintf( $format{$header}, $hyd_hash{$header}{'data'}[$i] )
            );
        }
    }
...
close(OUTPUT)
  or die "Could not close output file\n";
...
#
# RETURNS:
#	array of headers that were in the original data but are not in the final
#	data product
#-------------------------------------------------------------------------------
...
sub getOutputName {
    ...
    if ( -e $output_name ) {
        ...
        if ( !( $y_n =~ /y/i ) ) {
            ...
            unlink($output_name)
              or die "Could not write over file: $output_name\n";
        }
    }
    return $output_name;
}
