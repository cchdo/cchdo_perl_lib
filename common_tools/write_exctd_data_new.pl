#!/usr/bin/perl -w
#
#	WRITE_EXCTD_DATA_NEW:	a Perl subroutine that test-writes a CSV
#				formatted dump of the hash structures
#				for testing purposes.
#
#				This version of the routine can also handle
#				transmissometer and fluorometer data
#	
...
#
#	S. Diggs:	2007.08.08:	code now handles transmissometer and
#					fluorometer data.
#
#	M. Shen:	2008.07.24:	code flips transimissometer and fluorometer
#					positions and renamed FLUORM and TRANSM to
#					FLUOR and XMISS respectively.
#
#	S. Diggs:	2008.08.26:	backed out some of M. Shen's code changes
#					by renaming XMISS back to TRANS.  FLUOR stays
#					the same.  Also, commented out the code for
#					TRANS and FLUOR flags (uncalibrated).
#------------------->
#
sub write_exctd_data_new {
...
#STNNBR could be non-numeric
	...
	foreach my $param_head (@data_column)	{
		...
		if (($param_head ne 'FLUOR') and ($param_head ne 'TRANS'))	{
		 	print OUTPUT $param_head . '_FLAG_' . $flag_type . ',';
		}
	}
	print OUTPUT "\n";
	...
	for (my $i=0 ; $i <= $num_rec ; $i++)	{
		...
		#--> Write out data and flags for (4), probably FLUOR
		print OUTPUT sprintf($ctd_format{$data_column[4]}, 
				$ctd_hash{$data_column[4]}[$i]) , ',' 
			if (length($ctd_hash{$data_column[4]}[$i]));	
		#print OUTPUT sprintf($ctd_format{'FLAG'}, 
		#		$ctd_hash{$data_column[4] . '_FLAG'}[$i]) , ',', 
		#	if ($ctd_hash{$data_column[4] . '_FLAG'}[$i] > 0);	

		#--> Write out data and flags for (5), probably TRANS
		print OUTPUT sprintf($ctd_format{$data_column[5]}, 
				$ctd_hash{$data_column[5]}[$i]) , ',' 
			if (length($ctd_hash{$data_column[5]}[$i]));	
		#print OUTPUT sprintf($ctd_format{'FLAG'}, 
		#		$ctd_hash{$data_column[5] . '_FLAG'}[$i])
		#	if ($ctd_hash{$data_column[5] . '_FLAG'}[$i] > 0);	
